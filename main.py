...
t=Jinja2Templates(directory='templates'); d=DhanService(); engine=PrecisionEngine()
...
def scan(symbol:str):
    ...
    try:
        ...
        f['spot']=spot
        r=engine.evaluate(f,rows,lot_lookup=d.lot_size,capital=CAPITAL,risk_pct=RISK_PCT); r=guarded_result(symbol,r)
        ...
        return {'timestamp':datetime.now().isoformat(timespec='seconds'),'symbol':symbol,'expiry':exp,'spot':round(spot,2),'session_open':round(f.get('session_open',spot),2),'session_move':session_move,'session_pct':session_pct,
                ...
                'candles':f['candles'],'short_momentum_pct':round(f.get('short_momentum_pct',0),3),'live_feed':live,'mode':'LIVE WEBSOCKET + REST CONFIRMATION / MANUAL TRADE','auto_trade':False,'capital':CAPITAL,'risk_pct':RISK_PCT,'entry_window':entry_window_open(),**market_status(),**r}
